...
def main():
    ...
    B = 7.8  # Distance between the cameras [cm] (Lado a Lado)
    ...
    while (True):
        ...
        if ret_right == False or ret_left == False:
            ...
            if np.all(circles_right) == None or np.all(circles_left) == None:
                ...
                # ratio pixels to mm (quadrinho do cubo = 16mm)
                ratio_left = (radius_left*2) / 58.5
                ratio_right = (radius_left*2) / 58.5

                diameter_right_mm = (radius_right / ratio_right)*2
                diameter_left_mm = (radius_left / ratio_left)*2

                cv2.putText(frame_left, "Size: " + str(round(diameter_left_mm, 3)), (200, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                            (124, 252, 0), 2)

                cv2.putText(frame_right, "Size: " + str(round(diameter_right_mm, 3)), (200, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                            (124, 252, 0), 2)
